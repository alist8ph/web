...
class Model_Music extends PhalApi_Model_NotORM {
	...
	public function classify_list(){
		$list=DI()->notorm->users_music_classify->select("id,title,title_kr,img_url")->where("isdel=0")->order("orderno")->fetchAll();
		if(!$list){
			return 1001;
		}
	...
	public function music_list($classify,$uid,$p){
		...
		$list=DI()->notorm->users_music
			->select("id,title,title_kr,author,img_url,length,file_url,use_nums")
			->where($where)
			->order("use_nums desc")
			->limit($start,$nums)
			->fetchAll();
